...
data_loader = DataLoader(dataset, batch_size=n_prompt, shuffle=False, drop_last=True)
...
sae_out = sae(resid_pre)[0]
mse_loss_by_pos = (sae_out - resid_pre).norm(dim=(0, -1))
fig = px.line(
    mse_loss_by_pos.cpu().numpy(),
    width=800,
    title="MSE Loss by position",
    labels={"index": "Position", "value": "MSE Loss"},
)
# add a line at pos=128
fig.add_vline(x=sae.cfg.context_size, line_dash="dash", line_color="gray")
fig.show()
